...
def kyc_status(request, session_id=None, document_id=None):
    ...
    """
    if session_id:
        session_details = SessionDetails.objects.filter(session_id=session_id).first()
    elif document_id:
        personal_data = UserDetails.objects.filter(document_id=document_id).order_by("-id").first()
        session_details = personal_data.session_details if personal_data else None
    else:
    ...
    return Response({
        ...
        "document_id": personal_data.document_id,
        ...
        "updated_at": session_details.updated_at.isoformat()
    }, status=status.HTTP_200_OK)

@api_view(["GET"])
def kyc_details(request, session_id):
    """
    GET /kyc/api/details/<session_id>/
    Retorna todos los datos almacenados para una sesión KYC.
    """
    session_details = SessionDetails.objects.filter(session_id=session_id).first()
    
    if not session_details:
        return Response({"error": "No se encontró la sesión KYC"}, status=status.HTTP_404_NOT_FOUND)
    
    personal_data = session_details.personal_data
    
    # Construir una respuesta con todos los datos disponibles
    response_data = {
        "id": personal_data.id,
        "first_name": personal_data.first_name,
        "last_name": personal_data.last_name,
